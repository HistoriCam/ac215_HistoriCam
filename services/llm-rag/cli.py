...
def load_text_embeddings(df, collection, batch_size=500):
    ...
    hashed_buildings = df["building"].apply(
...
def chunk(method="char-split"):
    ...
    text_files = glob.glob(os.path.join(INPUT_FOLDER, "buildings", "*.txt"))
    ...
    for text_file in text_files:
        ...
        if text_chunks is not None:
            ...
            data_df["building"] = building_name
            print("Shape:", data_df.shape)
            print(data_df.head())

            jsonl_filename = os.path.join(
                OUTPUT_FOLDER, f"chunks-{method}-{building_name}.jsonl")
