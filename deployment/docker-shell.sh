...
if docker ps --format "table {{.Names}}" | grep -q "^${IMAGE_NAME}$"; then
    ...
    # Ensure docker config file exists for mounting
    touch "$(pwd)/docker_config.json"
    ...
    -v "$BASE_DIR/../services/vision":/vision \
    -v "$BASE_DIR/../services/llm-rag":/llm-rag \
